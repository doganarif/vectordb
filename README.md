...
| **Index & Search** |
| PUT                | `/libraries/{id}/index`              | Build/rebuild index  |
| POST               | `/libraries/{id}/searches`           | Search vectors       |
| **Admin**          |
| POST               | `/admin/snapshots`                   | Save snapshot        |
| PUT                | `/admin/snapshots`                   | Load snapshot        |
| **Utilities**      |
...
# Search for similar vectors
curl -X POST http://localhost:8000/libraries/{library_id}/searches \
  -H "Content-Type: application/json" \
  -d '{
    "vector": [0.1, 0.2, 0.3],
    "k": 10,
