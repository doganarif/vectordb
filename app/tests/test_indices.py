...
def test_kdtree_euclidean():
    ...
    r = client.post(
        f"/libraries/{lib_id}/searches", json={"vector": [0.0, 1.0, 0.0], "k": 1}
    )
...
def test_lsh_cosine():
    ...
    assert r.status_code == 202
    r = client.post(
        f"/libraries/{lib_id}/searches", json={"vector": [1.0, 0.0, 0.0], "k": 1}
    )
    assert r.status_code == 200
    assert len(r.json()["results"]) == 1
...
def test_metadata_filtering():
    ...
    r = client.post(
        f"/libraries/{lib_id}/searches",
        json={"vector": [0, 1, 0], "k": 5, "metadata_filters": {"lang": "en"}},
    )
    assert r.status_code == 200
